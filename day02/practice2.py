...
train_input, test_input, train_target,  test_target = train_test_split(iris_data, iris_target, test_size=0.3) # 학습용 7 : 테스트용 3 비율로 분할
...
dict, indexs = kn.kneighbors([[2.0, 0.5]])
plt.scatter(train_input[:,0], train_input[:,1]) # 학습용
plt.scatter(2.0, 0.5) # 예측값
...
# 품종을 다시 예측하세요.
new_scaled = scaler.transform([[2.0, 0.5]])
print(kn.predict(new_scaled))
# [단계 15] 스케일링 이후 최근접 이웃 시각화
# 스케일링된 데이터 기준으로
# 최근접 이웃들을 다시 시각화하세요.
ist, indexs = kn.kneighbors(new_scaled)
