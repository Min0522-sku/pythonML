...
target_species = df[df['Species'].isin(['Iris-setosa'])]
iris_length = target_species['SepalLengthCm'].values
iris_width = target_species['SepalWidthCm'].values
...
# 1. 비선형 데이터 표현력 향상: 단순 선형 회귀(1차 방정식)는 데이터의 흐름을 
#    직선으로만 표현할 수 있어 곡선 형태의 복잡한 데이터 분포를 반영하지 못합니다. 
#    반면, 특성을 제곱한 다항 회귀는 곡선(패러볼라) 형태의 회귀선을 그릴 수 있어 
#    데이터의 비선형적 흐름과 경향성을 더 정확하게 포착합니다.
# 2. 모델의 성능(설명력) 개선: 특성을 확장함으로써 잔차(오차)의 합을 줄일 수 있고, 
#    결과적으로 단순 선형 회귀보다 결정계수(R^2)를 높여 모델의 예측 능력을 향상시킵니다.